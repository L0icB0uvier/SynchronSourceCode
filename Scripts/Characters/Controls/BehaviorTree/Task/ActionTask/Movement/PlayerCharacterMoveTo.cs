...
namespace Characters.Controls.BehaviorTree.Task.ActionTask.Movement
{
	...
	public class PlayerCharacterMoveTo : Action
	{
		...
		private bool m_successfullyMoved = false;
		...
		{
			if (dynamicPath && ShouldRecalculatePath())
			{
				RequestPath();
			}

			if (m_successfullyMoved)
			{
		...
		public override void OnFixedUpdate()
		{
			if (m_followingPath)
			{
				...
				{
					AIController.Value.ChangeLookingDirection(Mathf.SmoothDampAngle(AIController.Value.LookingDirection, m_angleToStartPath, ref m_currentVelocity, smoothTime, 90, Time.fixedDeltaTime));	
				}
		...
		{
			if (location.Value == m_currentLocationToMoveTo || (location.Value - m_currentLocationToMoveTo).sqrMagnitude < requestPathThreshold * requestPathThreshold)
				return false;
			else return true;
		}
		...
		{
			m_currentLocationToMoveTo = location.Value;
		...
		private void MoveAI()
		{
			...
			if (m_path != null)
			{
				...
				if (AIController.Value.Rb2d.velocity.sqrMagnitude > .1f)
				{
